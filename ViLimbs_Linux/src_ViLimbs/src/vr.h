...
public:
    vr(void);
    virtual ~vr(void);
    virtual int createScene(aruco::CameraParameters camParams, unsigned char* buffer, std::string resourcePath,std::string assetFilename);
	...
	Ogre::Real mRotate;
	Ogre::Bone *bones[18];		//17 bones in the armature
	Ogre::Quaternion initR[18];
	Ogre::Quaternion r;
		Ogre::Real ratio;
	int currState;
	string emf,oldState;
