...
//-------------------------------------------------------------------------------------
int vr::createScene(aruco::CameraParameters camParams, unsigned char* buffer, std::string resourcePath="",std::string assetFilename="")
{
	/// INIT OGRE FUNCTIONS
    ...
    Ogre::Light* light1 = smgr->createLight("spotLight");
    light1->setPosition(-1.0f, 0.0f, -0.0f);
    Ogre::Light* light2 = smgr->createLight("PointLight");
    light2->setPosition(1.0f, 0.0f, 0.0f);
    ...
    ///hombre
    //Ogre::ResourceGroupManager::getSingleton().addResourceLocation("brazo27.zip", "Zip", "Popular");
    Ogre::ResourceGroupManager::getSingleton().addResourceLocation(assetFilename, "Zip", "Popular");
    ///mujer
    //Ogre::ResourceGroupManager::getSingleton().addResourceLocation("brazomujer1.zip", "Zip", "Popular");
    //Ogre::ResourceGroupManager::getSingleton().addResourceLocation("pierna54k_Noctrls.zip", "Zip", "Popular");
    //Ogre::ResourceGroupManager::getSingleton().addResourceLocation("pierna.zip", "Zip", "Popular");
    ...
    for(int i=0; i<MAX_MARKERS; i++) {
        ...
        //ogreEntity = smgr->createEntity(entityName, "Group1.mesh");
        ogreEntity = smgr->createEntity(entityName, "brazo.mesh");
        //ogreEntity = smgr->createEntity(entityName, "pierna.L.mesh");
        ...
        ogreNodeChild->translate(0,0.2,-0.2,Ogre::Node::TS_PARENT);
        ...
        ///mujer
        //ogreNode[i]->setScale(+scale+0.15, scale, scale+0.3);
...
void vr::setupArmature()
{
	...
	//bones[1] = skel->getBone("bn_tibia_l");
	bones[1] = skel->getBone("antebrazo");
	bones[1]->setManuallyControlled(true);
	//Ogre::Quaternion start, rot, end;
	//start=bones[1]->getOrientation();
	//initR[17]=bones[1]->getOrientation();
...
void vr::flexArm()
{
	...
	{
		rot.FromAngleAxis(Ogre::Degree(90), Ogre::Vector3::UNIT_Y);
		//end=Ogre::Quaternion::Slerp(ratio,start,initR[17],true);
...
void vr::closeHand()
{
	...
	if (ratio<0.5) 
	{
		 ...
		 * 
		 * 
		 * blender positive angle!!!!!
		3dsmax negative angle
		* 
		*/
		///roll -> unit_Z for fingers.2
		start=Ogre::Quaternion::IDENTITY;
...
bool vr::processUnbufferedInput(const Ogre::FrameEvent& evt)
{
	...
	else if(keyboard->isKeyDown(OIS::KC_E) || currState==2 || emf.compare(0,2,"FH") == 0)		/// flex hand
	{
		cout<<"------Entre a realizar "<<emf<<endl;
		flexHand();
	} else if(keyboard->isKeyDown(OIS::KC_R) || currState==3 || emf.compare(0,2,"EH") == 0)		/// extend hand
	{
		//emf="EH";
		extendHand();
	} else if(keyboard->isKeyDown(OIS::KC_C) || currState==8 || emf.compare(0,2,"CH") == 0)		/// close hand
	{
		//emf="CH";
		closeHand();
	...
	{
		pointerHand();
	} else if(keyboard->isKeyDown(OIS::KC_L) ||  currState==11 || emf.compare(0,2,"RH") == 0)		/// relax hand
	{
		//emf="NM";
		relaxHand();
	} else if(keyboard->isKeyDown(OIS::KC_F) ||  currState==13 || emf.compare(0,2,"FA") == 0)		/// relax hand
	{
		//emf="NM";
		flexArm();
	} 
